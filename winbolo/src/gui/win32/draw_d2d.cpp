...
/*********************************************************
*Name:              Draw
*Filename:          draw_d2d.cpp
*Author:            John Morrison
*Author (Direct2D): Nathan Bryant
*Creation Date:     13/12/98
*Last Modified:     2025-08-03
*Purpose:
*  System Specific Drawing routines (DXGI 1.4, Direct2D 1.3)
* Minimum OS is Windows 10. Tested with SDK 10.0.18362.0
* Supports flip-mode (zero-copy) Present, low latency
*********************************************************/
...
IDXGIFactory2* lpDxgiFactory = NULL;
ID2D1Factory1* lpD2dFactory = NULL;
ID2D1Device* lpD2dDevice = NULL;
ID2D1DeviceContext* lpD2dContext = NULL;
IDXGISwapChain1* lpSwapChain = NULL;
IWICImagingFactory* lpWICFactory = NULL;
ID2D1Bitmap1* lpDDSTiles = NULL;
ID2D1Bitmap1* lpDDSBackground = NULL;
ID2D1Bitmap1* lpDDSMessages = NULL;
ID2D1Bitmap1* lpDDSManStatus = NULL;
ID2D1Bitmap1* lpDDSPillsStatus = NULL;
ID2D1Bitmap1* lpDDSBasesStatus = NULL;
ID2D1Bitmap1* lpDDSBasesStatusBars = NULL;
ID2D1Bitmap1* lpDDSTankStatus = NULL;
ID2D1Bitmap1* lpDDSTankStatusBars = NULL;
ID2D1Bitmap1* lpDDSTankLabels = NULL;
ID2D1Bitmap1* lpDDSLGMButtons = NULL;

LPDIRECTDRAWCLIPPER lpDDClipper = NULL;

Windows::UI::Composition::Compositor bar;
ICompositorDesktopInterop foo;
...
bool drawSetup(HINSTANCE appInst, HWND appWnd) {
  ...
  DXGI_SWAP_CHAIN_DESC1 swapDesc;   /* Swap-chain description */
  ...
  /* Create all the alphabet soup devices, D2D DXGI etc */

  res = D3D11CreateDevice(NULL, D3D_DRIVER_TYPE_HARDWARE, appInst,
      D3D11_CREATE_DEVICE_SINGLETHREADED | D3D11_CREATE_DEVICE_BGRA_SUPPORT, featureLevels, 7, D3D11_SDK_VERSION,
      &lpD3dDevice, NULL, NULL);
  ...
  IDXGIDevice3* lpDxgiDevice;

  if (returnValue) {
      res = lpD3dDevice->QueryInterface(&lpDxgiDevice);
  ...
  if (returnValue) {
      res = lpDxgiDevice->GetAdapter(&lpDXGIAdapter);
  ...
  res = lpDXGIAdapter->GetParent(__uuidof(IDXGIFactory2), (void**)&lpDxgiFactory);
  ...
  if (returnValue == TRUE) {
    ...
    swapDesc.Width = zoomFactor * MAIN_BACK_BUFFER_SIZE_X * TILE_SIZE_X;
    swapDesc.Height = zoomFactor * MAIN_BACK_BUFFER_SIZE_Y * TILE_SIZE_Y;
    ...
    res = lpDxgiFactory->CreateSwapChainForHwnd(lpD3dDevice, appWnd, &swapDesc, NULL, NULL, &lpSwapChain);
    if (FAILED(res)) {
  ...
  /* Create D2D interface to swap-chain */
  if (returnValue) {
      res = lpSwapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), (void**)&tex);
  ...
  res = CoCreateInstance(CLSID_WICImagingFactory, NULL, CLSCTX_INPROC_SERVER, __uuidof(IWICImagingFactory),
      (void**)&lpWICFactory);
...
static HRESULT LoadBMPToD2D(HMODULE hModule, LPCTSTR lpName, LPCTSTR lpType, ID2D1Bitmap** ppBitmap)
{
    ...
    IStream* stream = CreateIStreamFromResource(hModule, lpName, lpType);

    hr = lpWICFactory->CreateDecoderFromStream(
        stream,
        NULL,
    ...
    if (SUCCEEDED(hr))
    {
        hr = lpWICFactory->CreateFormatConverter(&pConverter);
    }
    ...
    if (SUCCEEDED(hr))
    {
        hr = lpD2dContext->CreateBitmapFromWicBitmap(
            pConverter,
            NULL,
            ppBitmap
        );
...
static IStream* CreateIStreamFromResource(HMODULE hModule, LPCTSTR lpName, LPCTSTR lpType)
{
    ...
    return SHCreateMemStream((BYTE*)pResourceData, dwResourceSize);
}
...
void drawRedrawAll(HINSTANCE appInst, HWND appWnd, buildSelect value, RECT *rcWindow, bool showPillsStatus, bool showBasesStatus) {
  ...
  for (count=1;count<=total;count++) {
    auto ba = screenBaseAlliance(count);
  ...
  for (count=1;count<=total;count++) {
    auto ba = screenPillAlliance(count);
  ...
  for (count=1;count<=MAX_TANKS;count++) {
    auto ba = screenTankAlliance(count);
